	...
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
		    ...
		    % Sliko smo naredili tako, da so točke A, B, T in C vse enako oddaljene
				...
				% Risanje točke T
				 \fill[blue] (T) circle (2pt);%vtizlu previdno pousod ta podpicja
				 % Premica p %fill pobarva krogc fill pa zapolnjen krogec nardi
				 \draw[blue, very thick] (-2,0) -- (2,0) node[right] {$p$};
				\pause %dva miusa oomenta da naj bo ravn ačrta
				% Definicija pomožne točke A' in risanje krožnega loka k, ki se začne v A'
				\coordinate (A') at ({-\tt},{\td});%A' je tocka zacetka kroznega loka
				...
				\coordinate (T') at ({-\td},{\tt});%A' je tocka zacetka kroznega loka
				...
				\coordinate (T'') at ({\td},{\tt});%A' je tocka zacetka kroznega loka
				\draw[gray, thin] (T'') arc[start angle=120, end angle=240, radius=2] node[left] {\scriptsize $m$};
				\pause
