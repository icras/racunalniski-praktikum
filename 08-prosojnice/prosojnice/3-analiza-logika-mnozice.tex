\begin{frame}{Logika in množice}
	\begin{enumerate}
		\item
		Poišči preneksno obliko formule\\ %\land je logical and
		$\exists x : P(x) \land \forall x : Q(x) \Rightarrow \forall x:R(x).$
		\item 
		Definiramo množici $A=[2,5] in B=\{0,1,2,3,4,\ldots\} $%backslash da se zaviti oklepaji pojavijo
		V ravnino nariši:
		\begin{enumerate}
...
\begin{frame}{Kompleksna števila}
	\begin{enumerate}
		...
		%dfrac je display fraction size
	\end{enumerate}
\end{frame}